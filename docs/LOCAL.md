...
**Super для разработки** (ник `admin`, пароль `admin`, телефон-заглушка `79000000001` — ваш номер свободен для тестов):

```powershell
npm.cmd run local:super-dev
```

Вход: в поле «Логин» — `admin`, пароль — `admin`.
