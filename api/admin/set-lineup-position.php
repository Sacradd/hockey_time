...
try {
    ...
    $position = (int) ($body['position'] ?? $body['queue_position'] ?? 0);

    if ($gameId < 1 || $userId < 1 || $position < 1) {
        api_json_response(['ok' => false, 'error' => 'Укажите game_id, user_id и position (от 1)'], 400);
    }
    ...
    db_set_field_go_queue_position($pdo, $gameId, $rosterId, $game, $viewer, $userId, $position);
