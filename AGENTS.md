...
- **Страница игры (`GroupPage`):** один блок **«В игре»** (полевые + вратари), **«Резерв»**, **«Не будут»** — без подписей «полевые/вратари» и без лишних секций до старта голосования.
- **Порядок на игре:** кнопки голосования → блок админа (`vote-admin`: запуск голосования + оплата + составы) → списки состава внизу. **Не переносить** «Требование об оплате» под списки без явной просьбы.
- В «+» в очередь — только **полевые** из группы + гость (вратарей из группы в select не добавлять).
