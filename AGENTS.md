...
- **Страница игры (`GroupPage`):** **«В игре · вратари»** (без счётчика и без цифр очереди, макс. 2, свайп «Выбыл») → **«В игре · полевые»** (счётчик/цвет, «+», очередь) → **«Резерв · полевые»** → **«Не будут»** (общий список, ампула, свайп «В состав») → **«Ещё не ответили»** (после старта голосования). Третий вратарь (`goalie_reserve`) **не показывать**.
- **Порядок на игре:** кнопки голосования → `vote-admin` (оплата над списками) → секции состава. **Не переносить** оплату под списки без явной просьбы.
- В «+» — **полевые и вратари** из группы + гость (`add-goalie-to-game.php`, не больше 2 вратарей).
- После голоса «Буду» — `await refresh()` состава (не `load()` без await).
...
- Язык UI: **русский**.
- **Только то, что просили:** не менять порядок блоков, подписи статусов и API без согласования. Сомневаешься — спроси.
...
- **Git commit / push** — только по явной просьбе пользователя. **Перед commit/push** — сводка затронутых файлов (таблица: файл → зачем), подтверждение пользователя, затем commit (см. `.cursor/rules/pre-commit-summary.mdc`).
- **Деплой на prod:** `npm.cmd run build` → залить **содержимое `dist/`**; `npm run dev` на хостинг не заливается. `api/config.local.php` на сервере не перезаписывать.
